...
@Component({
  ...
  template: `
    ...
    >
        ...
        >
          <div class="relative">
            <button
              ...
              class="flex items-center gap-3 px-3 py-1.5 bg-white/5 rounded-full border border-white/10 hover:border-indigo-500/50 transition-all cursor-pointer"
            >
              <div
              ...
              <span class="text-[10px] lg:text-xs font-bold text-slate-300">{{
                activeOrg().name
              }}</span>
              <svg
            ...
            @if (isOrgDropdownOpen()) {
              <div (click)="isOrgDropdownOpen.set(false)" class="fixed inset-0 z-100"></div>
              <div
                class="absolute left-0 mt-2 w-64 bg-[#0b1120] border border-white/10 rounded-2xl p-2 shadow-2xl z-110 backdrop-blur-2xl animate-in fade-in slide-in-from-top-1 duration-200"
              >
                @for (org of availableOrgs(); track org.id) {
                  <button
                    (click)="switchWorkspace(org)"
                    [ngClass]="{ 'bg-white/10 text-indigo-400': isWorkspaceActive(org.id) }"
                  ...
                  >
                    <div
                      class="size-6 shrink-0 rounded border border-white/10 flex items-center justify-center text-[10px] uppercase"
                    >
          ...
          <div class="flex items-center gap-3 lg:gap-5">
            ...
            <div class="relative">
              ...
              >
                <div
                  class="size-8 rounded-full bg-linear-to-tr from-indigo-600 to-purple-600 border border-white/20 flex items-center justify-center text-xs font-black text-white uppercase"
                >
              ...
              @if (isProfileOpen()) {
                <div (click)="isProfileOpen.set(false)" class="fixed inset-0 z-100"></div>
                <div
                  class="absolute right-0 mt-2 w-56 bg-[#0b1120] border border-white/10 rounded-2xl p-2 shadow-2xl z-110 backdrop-blur-xl animate-in fade-in zoom-in duration-200"
                >
                  ...
                  <div class="py-1">
                    ...
                    @if (canSwitchToManager()) {
                      ...
                      >
                        <svg
                          class="size-4 text-emerald-500/60"
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
      ...
      @if (isAiOpen()) {
        <app-ai-assistant [(isOpen)]="isAiOpen" />
      }
    </div>
  `,
})
